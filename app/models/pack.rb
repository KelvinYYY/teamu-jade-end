class Pack
  ...
  # convenience method for access to packs collection
  def self.collection
   self.mongo_client['packs']
  end
  ...
  def self.all(prototype={}, sort={:population=>1}, offset=0, limit=100)
    ...
    Rails.logger.debug {"getting all packs, prototype=#{prototype}, sort=#{sort}, offset=#{offset}, limit=#{limit}"}
  ...
  # and returns instantiated Pack classes within a will_paginate
  # page
  def self.paginate(params)
    ...
    #get the associated page of Packs -- eagerly convert doc to Pack
    packs=[]
  ...
  def self.find id
    Rails.logger.debug {"getting pack #{id}"}

    doc=collection.find(:_id=>id)
                  .projection({_id:true, city:true, state:true, pop:true})
                  .first
    return doc.nil? ? nil : Pack.new(doc)
  end
